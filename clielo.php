...
// Locale override : doit être déclaré avant load_plugin_textdomain
add_filter( 'plugin_locale', function ( $locale, $domain ) {
    if ( $domain === 'clielo' ) {
        $override = get_option( 'clielo_locale', '' );
        if ( $override !== '' ) {
...
add_action( 'init', function () {
    load_plugin_textdomain( 'clielo', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
